...
import { Search, Database, FolderOpen, FileText, Settings, HelpCircle, Bot, Sparkles, Zap } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentUser, currentView, onViewChange }) => {
  const navigationItems = [
    ...
    { id: 'workspaces', label: 'Knowledge Base', icon: Database, description: 'Manage portfolios' },
    { id: 'search', label: 'Search Documents', icon: Search, description: 'Search knowledge base' },
    { id: 'agents', label: 'AI Agents', icon: Bot, description: 'AI-powered assistants' },
  ];
    ...
    <div className="w-64 bg-gray-50 border-r border-gray-200 flex flex-col h-full">
      ...
      <div className="flex-1 p-4 space-y-2">
        {navigationItems.map((item) => (
          ...
          >
            <item.icon className={`w-5 h-5 ${item.gradient && currentView === item.id ? 'text-orange-600' : ''}`} />
            ...
            {item.gradient && (
              <Zap className={`w-4 h-4 ${currentView === item.id ? 'text-orange-500' : 'text-orange-400'}`} />
            )}
          </button>
        ))}
      </div>
