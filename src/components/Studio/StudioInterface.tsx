...
function getCapabilities(model: string): string[] {
  ...
  if (model.includes('claude-3')) return [...base, 'Analysis', 'Complex Reasoning'];
...
function getProviderColor(providerId: string): string {
  ...
  return colors[providerId as keyof typeof colors] || 'from-gray-500 to-gray-600';
}
...
export const StudioInterface: React.FC = () => {
  ...
  const sendMessage = async () => {
    ...
    if (files.length > 0) {
      ...
      await sendQuickMessage(messageContent || `[${files.length} arquivo${files.length > 1 ? 's' : ''} enviado${files.length > 1 ? 's' : ''}]`);
    } else {
    ...
    try {
      ...
      await startStreaming(messageContent || `Analyze these ${files.length} file${files.length > 1 ? 's' : ''}`, {
            ...
            <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
              {messages.map((message, index) => (
                <MessageBubble
                  ...
                  onRegenerate={message.role === 'assistant' && index === messages.length - 1 && !isLoading && !isStreaming ? handleRegenerateResponse : undefined}
                />
              ))}
              ...
              <div className="mb-3 flex flex-wrap gap-2">
                {selectedFiles.map((file, index) => (
                  ...
                  >
                    ...
                    <span className="text-orange-500">({formatFileSize(file.size)})</span>
                    <button
            ...
            <div className="flex items-end space-x-4">
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder={selectedFiles.length > 0 ? `Analise estes arquivos com ${selectedModel.name}...` : `Converse com ${selectedModel.name}...`}
              ...
              <button
                onClick={sendMessage}
                disabled={(!message.trim() && selectedFiles.length === 0) || isLoading || isStreaming}
